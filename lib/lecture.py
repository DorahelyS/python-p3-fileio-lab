...
Introduction
When we work with computers, we need to do much more than execute code and browse the internet. If we want to 
save our work- or access data saved by other programs and computers- we need to use files.

A file is simply a container for storing information. This can be as plaintext in .txt files, or in special 
formatted files with .docx or .pdf extensions. There are also certain extensions- like .py- which give the 
computer and its users some hints about how to get the most out of the text contained inside, but which really 
label files that only store plaintext. For instance, you can open a .py file in an IDE like VSCode or execute it 
using the Python interpreter, but you can also open it in a simple text editor.

We've all worked with files many times before, but did you know that Python can open, write to, save, and close 
files as well? We call this process file I/O (input/output), and it's actually quite simple.

Opening a file
Before working with any file, we need to open it to access its contents. You can accomplish this in Python with the 
open() function:

text_file = open('file_directory/file_name.txt')
The only required argument for the open function is the path to the file in question. This can be an existing file 
or a path to a new one.

*** Python can create files very easily, but it takes some extra work to create directories. It's usually best to 
make all of your directories before using the open() function.

Now we can use the text_file variable to do operations on the file.

What operations can do? It depends on the mode we have opened the file in. The default mode is read, but we can 
specify different modes using a second argument into the open function.
...
If you do not specify the encoding Python will use a default encoding. The default is platform dependent 
(whatever locale.getpreferredencoding() returns), though it is typically utf-8. It is considered best practice to 
always define the encoding.

Closing a file
A file should be closed as soon as we are done using it. An open file can block other programs from using the file 
depending on the mode we open it in. You can close the file by using the close() method:
...
Inside the with code block we can use the variable text_file. When the block ends, Python will automatically call 
text_file.close(). The with statement guarantees the file will be closed even in the case of an exception.
